...
public class ResultsParser {
  ...
  private static class Parser {
    ...
    private ImmutableList<TournamentResults.Finish> makeFinishes() {
      ...
      if (results.ko() != null) {
        for (KToolResults.Level level : results.ko().levels()) {
          if (level.name().equals("1/1") || level.name().equals("FINALS-1-1")) {
    ...
    private KToolTeam getTeam(KToolPlay.Team team) {
      KToolTeam out = idToTeam.get(team.id());
      if (out != null) {
        return out;
      }
      if (!idToPlayer.containsKey(team.id())) {
        throw new RuntimeException("No player or team matched id " + team.id());
      }
    ...
    private SingleMatchEvent makeMatch(KToolPlay match) {
      SingleMatchEvent.Builder b = SingleMatchEvent.builder();
      KToolTeam team1 = getTeam(match.team1());
      KToolTeam team2 = getTeam(match.team2());
